...
| A 6DOF linear spring holds; it does not lift (E1-04) | **The lift is a separate, explicit step.** The spring drives the offset its two bodies had *at attach time* toward equilibrium, so a grip built while the box is on the floor holds it on the floor however stiff it is. E1-01 measured a box already in the air and so never met this. The host now places a fully crewed load into the carry pose and *then* builds the grips (`GrabDirector.Crew`). The snap that produces is a `ponytail:` with a named upgrade — move the hand to the grip, joint there, animate the hand back — deliberately left for **Gate 0** to judge, since "awkward or broken" is the word E1-10 collects |
